...
- [x] **FND-3 — App shell** — done 2026-09-24.
  **Built:** `RootLayout` (resolves the session once, above both shells, because the public header
  ...
  `Logo` (~~bundled, not from `app-config`~~ — since 2026-09-26 the admin's "Plan B logo" from `public/site-content`'s `branding`, swapped in only once loaded over the bundled mark, so it still never pops in), `LanguageSwitcher`, `siteNav.ts`
...
| 2026-09-26 | PUB-1 (part) | **Team row auto-rotates** (client instruction): one card every 3s, looping forever via a duplicated set (`components/shared/useLoopingCarousel.ts`; copies `aria-hidden`, links out of the tab order), native scroll kept. Stops on hover, keyboard focus, a tapped-open card, for one interval after the visitor swipes/scrolls, on a hidden tab; never rotates under reduced motion; no copies and no rotation when one set already fits. **Whole cards only**: 5 laptop / 3 tablet / 2 phone (`CAROUSEL_ITEM_CLASSES['5']`, replacing the team's `'5.5'` peek). **Social icons moved onto the photo**, shown on hover / focus / tap; the plate is name + role only (track `pb-14`→`pb-10`). Verified in headless Chrome: 5.0/3.0/2.0 cards, 246px steps every 3s, seamless wrap after 7 steps, hover holds still 8s then resumes, overlay opacity 0→1 on hover. |
| 2026-09-26 | — | **Header logo from the admin.** The website's `Logo` (public header, portal header, footer) now shows the "Plan B logo" uploaded under Settings › App Intro. New `branding: { logo_url }` block on `public/site-content` (`PublicBrandingResource`, one named field — the settings row holds the bank account; +2 tests). The bundled `logo.png` stays as first paint and fallback; the uploaded one replaces it only after it has loaded, and the last one that loaded is remembered per browser so returning visitors see it immediately. Hook lives in `siteContentQuery.ts` so the shells do not import the home page's content. **Asset issue:** the current upload is a shield on an opaque off-white square, so it shows as a light box on the navy header — it needs a transparent-background PNG. |
