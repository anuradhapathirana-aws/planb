...
| GET | `/student/courses` | Published programmes only, with a progress summary. Query: `search`, `category_id`, `per_page` (max 50), `page`. `category_id` of a top-level category returns its own courses **and every sub-category's**; a sub-category's returns just its own. A course whose category (or parent category) is inactive is left out — **unless this student is enrolled in it**. Each row carries `category_id`, `parent_category_id` and `category_name` (display only, in the student's language). Paginated. `search` matches the course name and its topic titles **in both languages regardless of `Accept-Language`** — most phones here are set to English, so a student reading in Sinhala routinely types the English word, and the course they are looking at has to come back. |
