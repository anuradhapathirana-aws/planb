...
| Method | Path | Auth / role | Notes |
|---|---|---|---|
| GET | `/admin/course-categories` | any admin role | Query: `search, is_active(1\|0), sort(name\|sort_order\|created_at), direction, per_page, page`. **A tree:** paginates top-level categories only, each with its sub-categories nested in `children` (in `sort_order`). With a filter, a parent is listed when it or any child matches; a matching parent keeps all its children, otherwise only the matching ones. Each row carries `parent_id`, `name_si`, `icon_image_url` and `programmes_count` (courses placed directly on it). |
| POST | `/admin/course-categories` | Super Admin, Content Manager | `{ parent_id?, name, name_si?, description?, icon?, selling_mode? }`. `selling_mode`: a main category takes `single` (default) or `bundle` (`inherit` is a 422); a sub-category also takes `inherit` (default — follow the main category) or a bundle of its own. Left out, a category moved between levels gets the sensible default. Rows carry `selling_mode`. `parent_id` null = main category; otherwise it must be a live **top-level** category (two levels only, 422 on `parent_id`). `name` is unique among its siblings, not globally. `icon` is one of `App\Enums\CourseCategoryIcon` or null (422 `Pick an icon from the list.` otherwise). `sort_order` is assigned server-side (appended last among its siblings). |
| GET | `/admin/course-categories/{category}` | any admin role | |
| PUT | `/admin/course-categories/{category}` | Super Admin, Content Manager | Same body as POST. Sending `icon: null` clears a chosen icon. A category with sub-categories cannot be given a parent (422). Moving to the top level drops an uploaded icon image. |
| DELETE | `/admin/course-categories/{category}` | Super Admin | **Soft-deletes** the category, its sub-categories and every course in them (each through the normal course delete, so Bunny videos are cleaned up). **422 `category`** — and nothing is deleted — while any student is enrolled in one of those courses, or has a `pending` / `awaiting_verification` order for one of those courses or for the category's bundle. 204 on success. |
| POST | `/admin/course-categories/{category}/icon-image` | Super Admin, Content Manager | Multipart `icon_image`: PNG only (content + extension), max 1 MB. Centre-cropped to 256×256 and re-encoded as PNG. **Sub-categories only** — 422 on a top-level category. Returns the category. |
| DELETE | `/admin/course-categories/{category}/icon-image` | Super Admin, Content Manager | Removes the uploaded icon; the picked `icon` (or the name guess) shows again. |
| POST | `/admin/course-categories/{category}/activate` | Super Admin, Content Manager | |
| POST | `/admin/course-categories/{category}/deactivate` | Super Admin, Content Manager | Switching a **parent** off hides its whole branch (sub-categories and their courses) from students without touching the children's own switches. Hidden courses leave the catalogue and cannot be bought (`isPurchasable()`), but **students already enrolled keep them**. |
| GET | `/admin/course-categories/{category}/course-order` | any admin role | The courses sitting **directly** in this category (not its sub-categories), drafts included, in their "Course 1, Course 2…" order. `{ data: [programme rows with position] }`. |
| PUT | `/admin/course-categories/{category}/course-order` | Super Admin, Content Manager | `{ programme_ids: [..] }` — **every** course in the category, each once, in the new order. Position in the array is the order; saved as `sort_order` 1..N. **422 `programme_ids`** for a partial list (a stale screen), **422 `programme_ids.N`** for a course from another category or a repeat. Returns the list as GET does. |
...
| Method | Path | Auth / role | Notes |
|---|---|---|---|
| GET | `/admin/course-programmes` | any admin role | Query: `search, course_category_id, status(draft\|published), sort(name\|sort_order\|created_at), direction, per_page, page`. Rows carry `topics_count` / `videos_count` and `position` ("Course N" within its own category, drafts counted; null on the single-course responses). |
...
| Method | Path | Notes |
|---|---|---|
| GET | `/student/course-categories` | The category tree behind Home's "Top Categories" row and the course filter: **every active top-level category**, in admin `sort_order` then name, **including categories with no published courses**, each with its active sub-categories in `children`. `{ data: [{ id, name, icon, icon_image_url, children: [...] }] }` via `StudentCourseCategoryResource`, none of the admin fields. `name` follows `Accept-Language` (Sinhala falls back to English). `icon` is null when no admin picked one — the app guesses from the name; `icon_image_url` (sub-categories only) wins over `icon` when set. |
| GET | `/student/course-categories/{id}` | The Category page. Visible categories only (404 otherwise), **unless the student owns a course in it**. `{ id, name, icon, icon_image_url, parent, children, courses: [course rows], courses_count, total_duration_seconds, owned_count, selling_mode, bundle }`. `children[].own_bundle` marks a sub-category sold as a bundle of its own. `bundle` (null when sold one by one) is the bundle this page sells — the category's own, or its main category's when it follows it — as this student would buy it: `{ category_id, name, courses_count, owned_count, remaining_count, remaining_price_cents, currency, is_available }`. |
| POST | `/student/course-categories/{id}/purchase` | Buy the rest of the course bundle this category's courses are sold in — its own, or its main category's when it follows it. 422 when they are sold one by one. `201 { status: 'payment_required', order }` with `order.items` (the frozen courses and prices; `amount_cents` is their sum), then the normal `/orders/{id}/card` or `/bank-transfer`; or `200 { status: 'enrolled', order: null }` when nothing is left to pay (all owned, or only free courses left — those are enrolled on the spot). Courses the student owns are never charged. 422 when the category does not sell as a bundle or is hidden. Order `item.type` is `category`. Throttled 20/min. |
| GET | `/student/courses` | Published programmes only, with a progress summary. Query: `search`, `category_id`, `per_page` (max 50), `page`. `category_id` of a top-level category returns its own courses **and every sub-category's**; a sub-category's returns just its own. A course whose category (or parent category) is inactive is left out — **unless this student is enrolled in it**. Each row carries `category_id`, `parent_category_id` and `category_name` (display only, in the student's language), and `position` — "Course N" within its own category, **published courses only** so there is no gap where a draft sits; numbering restarts in each sub-category. A suggested path, never a lock. With a top-level `category_id`, rows come grouped — the category's own courses first, then each sub-category's in sub-category order — each group in course order. Also `sold_individually` (false = a paid course in a bundle-mode category — `POST /courses/{id}/enrol` returns 422 for it) and `bundle` (`{ category_id, name, remaining_count, remaining_price_cents, currency }` — the bundle it is sold in; the `remaining_*` figures only on the detail response — or null). Paginated. `search` matches the course name and its topic titles **in both languages regardless of `Accept-Language`** — most phones here are set to English, so a student reading in Sinhala routinely types the English word, and the course they are looking at has to come back. |
...
| GET | `/student/app-config` | **public** | `{ logo_url, intro: { enabled, greeting_en, greeting_si, animation }, legal: { privacy_url, terms_url, account_deletion_url, support_email }, payments_enabled, app_version: { android, ios }, updated_at }`. `payments_enabled` only decides whether the app shows Buy / Enrol or "Coming soon" on paid items. Each `app_version` platform is `{ min_version, latest_version, store_url }` from `config/mobile_app.php` (`MOBILE_*` env); below `min_version` the app shows a blocking "Update required" screen, below `latest_version` a dismissible banner. Any null means no prompt. Read on launch, before sign-in. Branding only, never bank details. Rate-limited 60/min. |
...
| POST | `/student/orders/{order}/bank-transfer` | student | Multipart `reference_number` (**digits only, 4–30 long**, leading zeros kept — `digits_between:4,30`) + `receipt` (JPG/PNG/PDF, checked by real content and extension, max 5MB). Moves the order to `awaiting_verification`, **not** `paid`. Only one submission may sit in the queue at a time. **The slip is stored privately** under a random name; its type (and stored extension) comes from the bytes, not the file name, and images are re-encoded (EXIF/GPS removed, longest edge ≤ 2400px). On the student's order endpoints each payment's `receipt_url` is a **signed link valid 10 minutes** (`GET /payments/{payment}/receipt`), or null. |
...
| Method | Path | Auth | Notes |
|---|---|---|---|
| POST | `/payments/webhook/{gateway}` | **signature only** | Unauthenticated by necessity — the caller is the gateway's server, holding no session or token. The signature inside the payload is the authentication (`PaymentGateway::verifyWebhookSignature`, CLAUDE.md §7.9). Always answers 200 once the signature checks out, **including for duplicates**: a non-2xx makes the gateway retry forever. **`{gateway}` must be the configured `PAYMENT_GATEWAY`** — any other name, unknown or not, is **404** before a driver runs. The `sandbox` driver (which checks no signature) is registered only when `APP_ENV` is `local`/`testing`. Rate-limited 60/min. |
...
| GET | `/payments/sandbox/{payment}/confirm` | signed URL | Local stand-in for a hosted checkout page; 404s anywhere but `local`/`testing` (staging included). Posts a success back through the very same webhook path, so idempotency and settlement are genuinely exercised in development. |
...
| GET | `/admin/orders/{order}` | as above | Full order with every payment attempt. Each payment carries `has_receipt` — **no receipt URL**. |
| GET | `/admin/payments/{payment}/receipt-link` | as above (`OrderPolicy::view`) | `{ url, expires_at }` — a signed link to the slip, valid **10 minutes**. Called when "View receipt" is clicked, never cached. 404 if the payment has no slip. |
| GET | `/payments/{payment}/receipt` | **signed URL only** | Serves the slip with `X-Content-Type-Options: nosniff`, `Cache-Control: private, no-store`, `Content-Disposition: inline`. Expired or tampered signature → 403; slip not yet moved off the public disk → 404 (run `payments:migrate-receipts`). |
