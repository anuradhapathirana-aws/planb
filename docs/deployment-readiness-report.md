...
The server's `.env` is copied from **`backend/.env.production.example`**, which is committed and holds
every production value with placeholders for the rest. It is the single source for these settings;
`tests/Feature/EnvironmentTemplateTest.php` fails if its safe values drift. The fixed production
settings it carries:

| Setting | Value | Why |
|---|---|---|
| `APP_ENV` / `APP_DEBUG` | `production` / `false` | debug output exposes config and SQL |
| `LOG_STACK` / `LOG_LEVEL` | `daily` / `warning` | 14 days of files, no per-request noise |
| `SESSION_SECURE_COOKIE` | `true` | admin cookie only over HTTPS |
| `SESSION_ENCRYPT` | `true` | a database dump does not contain live admin sessions |
| `SESSION_LIFETIME` | `120` | minutes idle; an active admin is never signed out |
| `SESSION_DOMAIN` / `SANCTUM_STATEFUL_DOMAINS` | `.<DOMAIN>` / `admin.<DOMAIN>` | leading dot required |
| `TRUSTED_PROXIES` | `cloudflare` | real client IPs behind Cloudflare |
| `MAIL_MAILER` | `smtp` (Brevo) | never `log` |
| `QUEUE_CONNECTION` / `CACHE_STORE` | `database` | no Redis on this server |
| `PAYMENTS_ENABLED` | `false` | payments off at launch |
| `PAYMENT_GATEWAY` / `PAYHERE_SANDBOX` | `payhere` / `false` | `sandbox` does not exist in production |
| `PAYHERE_CHECKOUT_URL` | `https://www.payhere.lk/pay/checkout` | live, matches `PAYHERE_SANDBOX=false` |
