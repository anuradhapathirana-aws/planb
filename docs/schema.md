...
Records arrive two ways. An admin pre-loads them (FR-ADM-006 bulk CSV import of Student IDs) and the student later claims and fills in the record on the mobile app (FR-MOB-003/004) — or the student **self-registers with Google**, which creates the record outright. Either way the row is identical afterwards; `imported_by` is null and `google_sub` set on a self-registered student, which is how the two intakes are told apart. The emailed-code sign-in never creates anyone. Self-registration can be switched off with `STUDENT_GOOGLE_SIGNUP_ENABLED=false`.
...
| full_name | string, nullable | from the Google profile on self-registration, else null until CSV import or the student fills it in; **required** on the admin form |
...
| 2026-09-09 | Students may **self-register with Google**. `signInWithGoogle` creates the record when no `google_sub` or email matches, instead of rejecting; ID allocation moved to `StudentIdGenerator` so the admin form and signup share one sequence. No schema change. |
